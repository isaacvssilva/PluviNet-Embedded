...
esp_err_t sht30_init(i2c_master_bus_handle_t bus_handle, uint8_t endereco)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Falha ao adicionar dispositivo SHT30 no barramento I2C: %s", esp_err_to_name(ret));
        sht30.dev_handle = NULL;
        return ret;
    }
...
{
    if (sht30.dev_handle == NULL) return ESP_ERR_INVALID_STATE;
